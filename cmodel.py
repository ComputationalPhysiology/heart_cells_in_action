...
import numpy as np
import ctypes
from ctypes import c_int, c_long, c_ulong, c_double, c_char_p
import time
import os
...
class CModel:
    ...
    def solve(
        ...
        init_states: Optional[Dict[str, float]] = None,
    ):
        parameter_values = self.init_parameters()
        if parameters is not None:
            assert isinstance(parameters, dict)
        ...
        self.lib.init_state_values(u)
        u_values = np.zeros((num_steps + 1, u.shape[0]), dtype=np.float64)
        u_values[0, :] = u[:]

        if method == "FE":
            self.lib.ode_solve_forward_euler(
                u, parameter_values, u_values, t_values, num_steps, dt
            )
        elif method == "GRL1":
            self.lib.ode_solve_rush_larsen(
                u, parameter_values, u_values, t_values, num_steps, dt
